...
///////////////////////////////////////////////////////////////////////
//AVL TREE//
void BST::inorderStore(BinNode * root, vector<BinNode *> binNodes)
{
    if (root == NULL) return;
    inorderStore(root->left, binNodes);
    binNodes.push_back(root);
    inorderStore(root->right, binNodes);
}

BST::BinNode * BST::buildAvl(vector<BinNode *> binNodes, int start, int end) {
    if (start < end) return NULL;
    int mid = (start + end) / 2;

    BinNode * root = binNodes[mid];
    root->left = buildAvl(binNodes, start, mid - 1);
    root->right = buildAvl(binNodes, start, mid - 1);
    return root;
}

void BST::toavl()
{
    // tranverse and store each node into an array using inorder traversal
    vector<BinNode *> binNodes;

    inorderStore(myRoot, binNodes);
    int n = binNodes.size();
    myRoot = buildAvl(binNodes, 0, n - 1);
}